...
- **Environment Isolation via Folder Partitioning (`S3_FOLDER_PREFIX`):**
  - **Production:** `S3_FOLDER_PREFIX=prod` → all uploads are saved under `prod/user/...`, `prod/admin/...`
  - **UAT:** `S3_FOLDER_PREFIX=uat` → all uploads are saved under `uat/user/...`, `uat/admin/...`
...
- [ ] **Folder Partitioning:** Upload an asset on Prod → check Garage to verify key starts with `prod/` (e.g. `prod/user/...`); upload on UAT → starts with `uat/`.
