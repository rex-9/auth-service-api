...
class Client::UserVersionService
  class << self
    def create(user:, platform:, number:, version_code:)
      record = Client::UserVersion.find_or_initialize_by(user_id: user.id, platform: platform)
      record.number = number
      record.build_number = parse_version_code(version_code)
      record.version = Client::Version.lookup_by_number(number)
