...
RSpec.describe "V1 User Versions API", type: :request do
  ...
  describe "POST /v1/versions/user-version" do
    it "requires authentication" do
      post "/v1/versions/user-version", params: { user_version: { version: "1.4.0" } }

      expect(response).to have_http_status(:unauthorized)
      expect(UserVersion.count).to eq(0)
    end

    it "creates a UserVersion for the current user and platform" do
      version = create(:version, :published, number: "1.4.0")

      post "/v1/versions/user-version",
      ...
      record = user.user_versions.find_by!(platform: AuthConstants::Platform::ANDROID)
    ...
    it "updates the existing user version for the same user and platform" do
      ...
      existing = user.user_versions.create!(
      ...
      expect(user.user_versions.count).to eq(1)
