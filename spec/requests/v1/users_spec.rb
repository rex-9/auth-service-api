...
RSpec.describe "V1 Users API", type: :request do
  ...
  describe "GET /v1/users/current" do
    it "returns the authenticated user attributes" do
      ...
      expect(response_data.dig("user", "iam")).to include(
        "is_admin",
        "is_super_admin",
        "roles",
        "admin_roles",
      ...
      expect(response_data.dig("user", "iam", "is_admin")).to be(true)
      expect(response_data.dig("user", "iam", "is_super_admin")).to be(false)
      expect(response_data.dig("user", "iam", "admin_roles").map { |r| r.dig("attributes", "name") }).to include("feedback_admin")
      expect(response_data.dig("user", "iam", "admin_permissions").map { |p| p.dig("attributes", "name") }).to include("read_feedbacks")
    end
    ...
    it "does not require an IAM permission to read the current user's own document" do
      ...
      expect(response).to have_http_status(:ok)
      expect(response_data.dig("user", "iam", "is_admin")).to be(false)
      expect(response_data.dig("user", "iam", "roles")).to be_empty
      expect(response_data.dig("user", "iam", "permissions")).to be_empty
    end
  end
  ...
  describe "PUT /v1/users/current" do
    ...
    it "does not require an IAM permission to update the current user's own document" do
      ...
      expect(response).to have_http_status(:ok)
      expect(response_data.dig("user", "name")).to eq("Nope")
    end
  end
end
